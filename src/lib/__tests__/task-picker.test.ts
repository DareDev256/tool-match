...
function makeItems(overrides: Partial<ToolMatchItem>[], base?: Partial<ToolMatchItem>): ToolMatchItem[] {
  return overrides.map((o, i) => ({
    ...
    toolOptions: ["chatgpt", "dont-use-ai"],
    ...base,
    ...o,
  }));
}
...
describe("diversePick", () => {
  ...
  it("handles pool with zero 'Don't Use AI' items gracefully", () => {
    const noNoAi = makeItems(
      Array.from({ length: 20 }, (_, i) => ({ id: `item-${i}`, answer: "chatgpt" }))
    );
  ...
  it("caps 'Don't Use AI' floor at available count when pool has only 1", () => {
    const pool = makeItems([
      { id: "noai-1", answer: "dont-use-ai" },
      ...Array.from({ length: 15 }, (_, i) => ({ id: `reg-${i}`, answer: "chatgpt" })),
    ]);
  ...
  it("proportionally represents categories — no single category hogs all slots", () => {
    ...
    const pool = makeItems([
      ...Array.from({ length: 10 }, (_, i) => ({ id: `text-${i}`, category: "text-tasks" })),
      ...Array.from({ length: 10 }, (_, i) => ({ id: `vis-${i}`, category: "visual-tasks" })),
    ]);
